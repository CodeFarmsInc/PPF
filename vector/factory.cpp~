...
#else // LINUX
    int creat(const char *,long unsigned int);
#endif // LINUX
    int close(int);
...
public:
    PersistPage(long pageNum,long pageSz,long objSize,void *oneObj,
...
private:
    ...
    PersistPage *prev;
    long pageNo;       // pages are numbered
...
// always the page which has been unused for the longest time.
//
...
class PersistPager {
public:
    void* getPtr(long ind); // ind=object address within the pager file
    PersistPager(long objSize,void *validObj,long pageSize,long pagesInMem,
          long totalSize,char* className,PersistHeader *ph,long firstObject,
          int needsUpd,hiddenType hn,long *freeStrings,int rd);
    ~PersistPager();
    void startPager();
    void closePager();
    void debugPages(); // printout of the internal data structures
    void debugCheck(); // automatic crosscheck, no printout if OK
    long getPageSz(){return pageSz;}
    int getNumPages(){return numPages;}
private:
    void markUpdated(PersistPage *pp);
    PersistPage *ptrPart(long k){ // pointer information stored in pageArr[]
                     return (PersistPage*)((long)(pageArr[k]) & ptrMask);}
    long updPart(long k){         // update information stored in pageArr[]
                     return (long)(pageArr[k]) & updMask;}
    hiddenType myNew;
    ...
    void growArr(long pageNo); // grow by multiple of 2 to fit the given pageNo
    void sortPages();
    PersistPage* getPage(long pageNo); // get new/old page, assign it pageNo
    void remove(PersistPage *pp);
    void insertAsHead(PersistPage *pp);
    long cmp(PersistPage*p1,PersistPage *p2); // compare two page numbers

    PersistHeader *pHeader; // outside structure where the header is stored
    long *freeStr;   // outside structure for free strings, NULL for classes
    long objSz;      // size of one object in bytes
    void *oneObj;    // valid object of the type for which this pager is used
    long pageSz;     // size of one page in bytes, multiple of objSize
    long numPages;   // number of active pages actually used
    long maxActive;  // maximum number of pages in memory at the same time 
    long maxNumPag;  // estimated maximum number of pages, not critical
    long fileFill;   // number of bytes currently used in the file
    ...
    long firstObj;    // starting address of the first object
    static long ptrMask;
    static long updMask;
...
long PersistPager::ptrMask=(long)(-2);
long PersistPager::updMask=(long)(1);

// ----------------------------------------------------------------------
//                     IMPLEMENTATION: PersistHeader 
// ----------------------------------------------------------------------
PersistHeader::PersistHeader(long firstObj){freeObj=freeArr=root=0; 
                  current=firstObj;}
...
void PersistFactory::setRoot(long rt){info->root=rt;}

long PersistFactory::getRoot(){return info->root;}
...
void PersistFactory::prtTable(){
    ...
    for(i=0; i<classCount; i++){
        cout<<"PersistFactory::table["<<i<<"]="<<
              (long)(table[i])<<" class="<< table[i]->classN;
        k=table[i]->tableInd;
...
void PersistFactory::freeArr(long ind){
    long* next=(long*)(getPtr(ind));
    *next=info->freeArr;
    info->freeArr=ind;
}
...
// ------------------------------------------------------------------------
PersistFactory::PersistFactory(int *cIndex,long objSize,void *validObj,
                       long pageSize,long pagesInMem,long totalSize,
...
void PersistFactory::synchClasses(){
    ...
    printf(
  "Persistent Pointer Factory Ver.3.0, Copyright(C) 1999 Code Farms Inc.\n");
...
// ----------------------------------------------------------------------
void* PersistFactory::getPtr(long ind){ return pPager->getPtr(ind);}
// ----------------------------------------------------------------------
...
// ----------------------------------------------------------------------
void* PersistFactory::newObj(){ void *v; long *vv;
    if(info->freeObj>0){
        v=getPtr(info->freeObj);
        lastInd=info->freeObj;
        vv=(long*)v; info->freeObj=(*vv);
    }
    else {
...
// ----------------------------------------------------------------------
void PersistFactory::freeObj(long ind){
    long i=ind;
    void* v=getPtr(i);
    if(i+objSz==info->current)info->current=i;
    else { long* s=(long*)v; *s=info->freeObj; info->freeObj=i;}
}
// ----------------------------------------------------------------------
// Allocate an array of pointers, return the index for the beginning.
...
// ----------------------------------------------------------------------
long PersistFactory::newArr(long size){ 
    long k,*last,next,*p;  void *v;

    for(k=info->freeArr, last=NULL; k>0; ){
        p=(long*)getPtr(k-objSz);
        if((*p)>=size && (*p)<2*size)break;
        last=(long*)(getPtr(k)); k=(*last);
    }
    if(k==0){ // new array is needed
        k=info->current+objSz; info->current=k+size*objSz;
        p=(long*)(getPtr(k-objSz)); *p=size;
        ...
        p=(long*)(getPtr(k)); next=(*p);
        if(last){ *last=next; }
        else { info->freeArr=next; }
    }
    v=getPtr(k); // all this may not be eventually needed
    lastInd=k; lastPtr=v;
    return k;
}
// ----------------------------------------------------------------------
...
// intensive.
// ----------------------------------------------------------------------
long PersistFactory::getInd(void *ptr){ 
    if(ptr==lastPtr && lastTableInd==tableInd)return lastInd; 
    cout<<"error: keeping raw pointer="<<(long)ptr<<" for too long\n";
    return 0;
}
...
// ----------------------------------------------------------------------
PersistPage::PersistPage(long pageNum,long pageSz,long objSize,void *oneObj,
                         PersistPage *masterPage){
    char *p,*s; long i,k;
...
void PersistPage::debugPrt(PersistPager *pp){
    ...
    sz=sizeof(long);
    pageSz=pp->getPageSz();
    for(k=0, p=(long*)where; k<pageSz; k=k+sz, p++){
        printf("%ld ",*p);
    }
    printf("\n");
}
...
// ----------------------------------------------------------------------
inline long PersistPager::cmp(PersistPage *p1,PersistPage *p2){
    if(p1 && !p2)return -1;
    if(!p1 && p2)return 1;
...
// been unused for the longest time, move its content to disk, and
// re-assign its pageNo (both within the PersistPage class, and in pageArr).
// ----------------------------------------------------------------------
PersistPage* PersistPager::getPage(long pageNum){
    PersistPage *pp,*mp; long k;
...
// ----------------------------------------------------------------------
void* PersistPager::getPtr(long address){
    long pageNo,offset; PersistPage *page;
...
// ----------------------------------------------------------------------
void PersistPager::growArr(long pageNo){
    PersistPage **newArr;

    long oldNumPag=maxNumPag;
    for( ; maxNumPag<=pageNo; maxNumPag=2*maxNumPag)continue;
    if(oldNumPag<maxNumPag){
        newArr=new PersistPage* [maxNumPag];
        if(newArr){
            for(long i=0; i<maxNumPag; i++){
                if(i<oldNumPag) newArr[i]=pageArr[i];
                else            newArr[i]=NULL;
            }
            delete[] pageArr;
            pageArr=newArr;
...
void PersistPager::pageToMemory(PersistPage *pp){
    long i,k,address,numBytesReadIn;
    char *p,*t,s;
...
void PersistPager::startPager(){
    ...
    newFile=0; // will record whether old/new data file is used
    if(IOperm==0)Oflag=O_RDONLY; else Oflag=O_RDWR;
    if(IOperm==0)Sflag=S_IREAD; else Sflag=S_IREAD | S_IWRITE;

#ifdef MAC
    fileHandle=open(fileName,Oflag);
    if(fileHandle<0 && IOperm>0){
        fileHandle=open(fileName,O_CREAT | Oflag); newFile=1;
    }
#else
#ifdef __VMS
    fileHandle=open(fileName,Oflag, 0); 
    if(fileHandle<0 && IOperm>0){
        fileHandle=open(fileName,O_CREAT | Oflag, 0); newFile=1;
    }
#else
#ifdef DOS
    fileHandle=open(fileName,Oflag | O_BINARY,Sflag);
    if(fileHandle<0 && IOperm>0){
        fileHandle=open(fileName,O_CREAT | Oflag | O_BINARY,Sflag);
        newFile=1;
    }
#else
    fileHandle=open(fileName,Oflag,Sflag);
    if(fileHandle<0 && IOperm>0){
        fileHandle=open(fileName,O_CREAT | Oflag,Sflag);
        newFile=1;
    }
#endif
#endif
    ...
    for(long i=0; i<maxNumPag; i++)pageArr[i]=NULL;
}
...
// ----------------------------------------------------------------------
// Read in only the first two longs, and fix up the header as if remaining
// values are not used.
// When newFile==1, Read the pager header which starts at the begining 
// of the first page.
// When newFile==0, initialize all the values to an empty data set.
...
void PersistPager::loadStringHeader(){
    ...
    if(newFile){
        ...
        freeStr[0]=pageSz;
        freeStr[1]=firstObj;
        for(i=2, n=pageSz/sizeof(long); i<n; i++)freeStr[i]=0;
    }
    else {
        if(fileHandle>=0){
            lseek(fileHandle, 0, 0); // 0=offset mode from the beg.of file
...
// ----------------------------------------------------------------------
PersistPager::PersistPager(long objSize,void *validObj,long pageSize,
  long pagesInMem,long totalSize,char* className,PersistHeader *ph,
  long firstObject,int needsUpd,hiddenType hn,long *freeStrings,int rd){
    ...
    if(!objSz)printf("error: wrong input for PersistPager constructor\n"); 
        ...
        else { strcpy(fileName,className); strcat(fileName,".ppf");}
    }
    pageArr=NULL;
    tail=NULL;
    fileHandle= -1;
...
void PersistString::startPager(long pgSz,long maxPgs,long totMem,int rd){
    static char ch='\0';
    long n;

    pageSz=pgSz;
    n=pgSz/sizeof(long); 
    if(n*sizeof(long) !=pgSz){
        n++;
        pageSz=n*sizeof(long);
        printf("warning: page size for strings adjusted to %d\n",pageSz);
    }
    ...
    pPager=new PersistPager(1,&ch,pageSz,maxPgs,
                 totMem,"pstring",info,pageSz,0,NULL,freeStr,rd);
...
// from some long string (if available).
// ----------------------------------------------------------------------
char* PersistString::newStr(long sz){
    long *longPtr; void *nxtPtr; char *p;
...
void PersistString::delString(){
    long sz,*longPtr; int isFree;

    if(index==0)return;
    longPtr=(long*)(pPager->getPtr(index-sizeof(long)));
    sz=(*longPtr);
    if(index+sz+pageSz == diskSz){ // last string on disk, first page has index
        diskSz=index-sizeof(long);
        for(;;){
...
// ----------------------------------------------------------------------
void PersistString::releaseFree(long ind,long sz){
    long next,prev,*longPtr,i; char *cp;

    i=sz/sizeof(long);

    // same page, no need to call getPtr() twice
    cp=(char*)(pPager->getPtr(ind)); longPtr=(long*)cp; prev=(*longPtr);
    cp=cp-sizeof(long);              longPtr=(long*)cp; next=(*longPtr);

    if(prev==0)freeStr[i]=next;
    else {
        longPtr=(long*)(pPager->getPtr(prev-sizeof(long))); *longPtr=next;
    }
    if(next){
        longPtr=(long*)(pPager->getPtr(next)); *longPtr=prev;
    }
}
...
// where after some round-ups i=stringSize/sizeof(long).
//
...
// than 3+sizeof(long) (string of 4 char or less), it is added to the 
// string. In the same vein, if there is no free string for given i,
// we also try i+1.
// ----------------------------------------------------------------------
void PersistString::getFreeString(long sz){
    char *vp; long *longPtr,next,prev,i,k,size,address;
    ...
    longPtr=(long*)vp;                prev=(*longPtr);
    longPtr=(long*)(vp-sizeof(long)); next=(*longPtr);
    freeStr[i]=next;
    *longPtr=i*sizeof(long);
    if(next>0){
...
// ----------------------------------------------------------------------
void PersistString::getNewString(long size){
    long lastChar,*longPtr,tmp,leftOver,sz,stringToFree; 

    sz=roundSz(size);
    if(sz==0){index=0; return;}
    lastChar=diskSz+sz+sizeof(long)-1;
    if(diskSz/pageSz != lastChar/pageSz){ // does not fit into the current page
        tmp=(diskSz/pageSz + 1)*pageSz - diskSz;
        longPtr=(long*)(pPager->getPtr(diskSz));
        if(!longPtr)return;
        *longPtr=tmp-sizeof(long);
        stringToFree=diskSz+sizeof(long);
        diskSz=(diskSz/pageSz + 1)*pageSz;
        setFree(stringToFree);
    }

    // If the leftover of the page is too small (<3*sizeof(long)), just
    // make this string bigger.
    leftOver=((diskSz/pageSz)+1)*pageSz - (diskSz+sz+sizeof(long));
    if(leftOver < 3*sizeof(long))sz=sz+leftOver;
    
    longPtr=(long*)(pPager->getPtr(diskSz));
    if(longPtr){
        *longPtr=sz;
        index=diskSz+sizeof(long);
        diskSz=diskSz+sz+sizeof(long);
        info->current=diskSz;
    }
    else index=0;
...
// ----------------------------------------------------------------------
void PersistString::setFree(long ind){
    long *longPtr,i; char *vc;

    setTail(ind,1); // must be called before changing the header, 1=free string
    vc=(char*)(pPager->getPtr(ind));
    longPtr=(long*)(vc-sizeof(long));
    if(!longPtr)return;
    i=(*longPtr)/sizeof(long);
    *longPtr=freeStr[i];
    longPtr=(long*)(pPager->getPtr(ind));
    *longPtr=0;
    freeStr[i]=ind;
}
...
// ----------------------------------------------------------------------
void PersistString::setTail(long ind,int isFree){
    long *longPtr,sz,isFreeFlag; unsigned char *cp,c[4];
    ...
    *longPtr=0;
    ...
    longPtr=(long*)(pPager->getPtr(ind-sizeof(long)));
    if(!longPtr)return;
    sz= *longPtr;
    cp=(unsigned char*)longPtr; 
    if(sz<=shortTail){ // tail is just 1 byte
        cp=cp+sizeof(long)+sz-1; // position of the last byte
        *cp=(unsigned char)(sz/2 + isFree); // sz is a multiple of 4, compacted
        ...
        longPtr=(long*)cp;
        *longPtr=sz/4;
        if(*longPtr>=isFreeFlag){
            cout<<"ERROR setTail() sz="<<sz<<" too big\n"; cout.flush();
        }
        else if(isFree) *longPtr=(*longPtr)+isFreeFlag;
    }
}
...
// it is free.
// ----------------------------------------------------------------------
long PersistString::getTailSize(long lastByte,int *isFree){
    unsigned char *cp,k; long *longPtr;
    ...
    *isFree=(*cp) & one;
    ...
    if(k) return (long)(k+k);      // 1 byte tail
    longPtr=(long*)(cp-3); // 4 byte tail
    return (*longPtr)/remain;
}
...
// ----------------------------------------------------------------------
long PersistString::roundSz(long size){
    long sz,n;

    if(size<shortTail)sz=size+1; else sz=size+sizeof(long);
    n=sz/sizeof(long);
    if(n*sizeof(long) != sz) sz=(n + 1)*sizeof(long);
    if(sz<2*sizeof(long))sz=2*sizeof(long);

    if(sz+sizeof(long)>pageSz){
...
// ----------------------------------------------------------------------
char*  PersistString::getPtr(long ind) { return (char*)(pPager->getPtr(ind));}
...
// ----------------------------------------------------------------------
long  PersistString::getInd(char *s){
    if(lastPtr==s)return lastInd;
    cout << "error: calling getInd() after allocating other objects\n";
    return 0;
}
...
// ----------------------------------------------------------------------
long PersistString::allocSize(){
    long sz;
    long* longPtr=(long*)(pPager->getPtr(index-sizeof(long)));
    sz=(*longPtr);
...
void PersistString::debugFree(int prtFlg){
    long i,size,sz,address,*longPtr,lastByte,next; char *vc;
    int isFree;
    ...
    for(i=2; i<freeSz; i++){
        ...
        for(address=freeStr[i]; address; address=next){
            ...
            longPtr=(long*)(vc-sizeof(long)); next=(*longPtr);
...
// Most of this stuff really belong to file factory.h, but since my intention
// is to hide the Pager when the binary copy is used, I placed it here.
// 
// (1) The free list uses long indexes, not pointers. The end of list is -1.
...
//     including the possible '\0' character at the end may be longer than
//     (pageSize - 8).
...
//                        divided by 4;
// When implementing this, note that when overlaying char[4] and long,
// the lowest bit of the long is in char[0].
//
// This implies that the most efficient string sizes are 7,11,15,19,...,507,
// for which the internal tail info is only 1 byte. For longer strings,
...
//                 which from the user point of view is the longest string
//                 allowed, pageSize - 8.
//
...
// perhaps a slightly longer string (by much longer?) if it is available.
// 
//                                      Jiri Soukup, Nov.26/98
//
// ----------------------------------------------------------------
// WARNINGS: 
// - When class names are long, file names will be long!! What operating
